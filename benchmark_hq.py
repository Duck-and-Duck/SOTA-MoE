...
print("=========================================================================")
print("      GERCEKCI VE TAVIZSIZ BILANCO V21 (HQ DATASET VALİDASYONU)          ")
...
vetter = AstroNetHQ().to(device)
vetter.load_state_dict(torch.load("astronet_hq.pt", map_location=device))
vetter.eval()
...
from train_hq_data import generate_mandel_agol_transit, generate_realistic_binary, generate_harvey_red_noise
...
# 100 Gezegen Testi (Mandel-Agol Limb-Darkened U-Profili)
for _ in range(100):
    ...
    f_tensor = torch.tensor(flux, dtype=torch.float32, device=device)
    
    g, l = phase_fold_and_zscore(t_sim, f_tensor, p_test, 0.0)
    with torch.no_grad():
...
# 100 İkili Yıldız Testi (Sekonder Tutulmalı Gerçekçi V-Profili)
for _ in range(100):
    noise = generate_harvey_red_noise(3000)
...
print("\n=========================================================================")
print("               DURUST VE GERCEKCI BILIMSEL BILANCO (V21)                 ")
print("=========================================================================")
