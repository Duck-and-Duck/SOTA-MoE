...
print("="*85)
print("   OSTE-MoE V30: KUSURSUZ 4-SEKTOR BILIMSEL BENCHMARK SUITE")
print(f"--> HESAPLAMA BIRIMI: {device.upper()} | STANDART: Shallue & Vanderburg / Seager (2003)")
print("="*85)
...
model.load_state_dict(torch.load("astronet_master.pt", map_location=device))
model.eval()
print("--> [TAMAM]: Master Model Belleğe Alındı.\n")
...
def phase_fold_normalized_gpu(time_t, flux_t, period, t0, duration):
...
def extract_advanced_tce(time_arr, flux_arr):
    ...
    # İkincil Tutulma Taraması: Faz 0.25, 0.50, 0.75
    phase_0 = ((time_arr - final_t0 + 0.5 * final_p) % final_p) - 0.5 * final_p
    in_tr = np.abs(phase_0) < (final_dur / 2.0)
    primary_depth = float(1.0 - np.nanmedian(flux_arr[in_tr])) if np.sum(in_tr) > 3 else 1e-4

    sec_depths = []
    for offset in [0.25, 0.50, 0.75]:
        s_ph = ((time_arr - final_t0 - offset * final_p + 0.5 * final_p) % final_p) - 0.5 * final_p
        s_m = np.abs(s_ph) < (final_dur / 2.0)
        if np.sum(s_m) > 3:
            sec_depths.append(float(1.0 - np.nanmedian(flux_arr[s_m])))
    max_sec = max(sec_depths) if len(sec_depths) > 0 else 0.0
...
def infer_target(time_arr, flux_arr):
    ...
    g_v, l_v, measured_depth = phase_fold_normalized_gpu(t_t, f_t, tce["p"], tce["t0"], tce["dur"])

    # Seager (2003) Trapezoid Taban Oranı (Dip -1.0 üzerinden)
    ...
    # Astrofiziksel Karar Motoru
    # 1. Gürültü Tahliyesi: Transit derinliği gürültünün 2.5 katından küçükse veya SNR yetersizse
    if tce["snr"] < 6.5 or tce["depth"] < 2.0 * tce["scatter"]:
        final_class = "NON_PLANET"
    # 2. İkili Yıldız Elemesi: İkincil tepe > %35 VEYA V-Şekli Sivri Apeks (flatness < 0.28 ve Derinlik > 3500 ppm)
    elif tce["sec_ratio"] >= 0.35 or (flatness < 0.28 and tce["depth"] >= 0.0035) or probs[2] >= 0.40:
        final_class = "BINARY"
    # 3. Gezegen
    else:
        final_class = "PLANET" if probs[1] >= 0.35 else "BINARY"

    return final_class, probs, tce, flatness
...
for tgt in p2_targets:
    ...
    if tgt["type"] == "PLANET":
        ...
        f_sim[sec_m] -= (tgt["depth"] * 0.5) * np.maximum(0.0, 1.0 - np.abs((t_sim[sec_m] % tgt["p"])/0.05))
...
for _ in range(50):
    ...
    c_pred, _, _, _ = infer_target(t_sim, pure_noise)
...
for tgt in REAL_TARGETS:
    try:
        ...
        c_pred, probs, tce, flt = infer_target(raw_t, raw_f)
        is_hit = (c_pred == tgt["true"])
        if is_hit: real_correct += 1

        print(f"  * {tgt['name']:<20} | Gercek: {tgt['true']:<10} | Karar: {c_pred:<10} | P: {tce['p']:6.3f}g | Flat: {flt:.2f} -> {'[✓ GECTI]' if is_hit else '[✗ KALDI]'}")
